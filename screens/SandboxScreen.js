...
import { View, ScrollView, Text, StyleSheet, ActivityIndicator, Image, TouchableOpacity, TextInput } from 'react-native'
...
export default class SettingsScreen extends React.Component {
  ...
  static navigationOptions = ({navigation}) => {
    return {
      ...
      headerRight: () => <Icon name="ios-search" onPress={() => NavigationService.navigate('Search')} style={{color: 'white', fontSize: 30, marginRight: 10, marginTop: 5}}/>
    }
  }
  ...
  render() {
    if(this.state.isLoading) {
      return <View style={{flex: 1, justifyContent: 'center', alignItems: 'center'}} size='large'><ActivityIndicator /></View>
    } else {
        ...
        return (
          <TouchableOpacity key={obj.id} activeOpacity={0.5}
            ...
            <View style={{flex: 1, marginTop: 10, width: 150}}>
              <Text style={{alignSelf: 'center', color: 'white'}} numberOfLines={2}>{obj.title.romaji}</Text>
            </View>

          </TouchableOpacity>
        )
      })
      let trendinganime = this.state.trendinganimedata.data.Page.media.map(obj => {
        return (
          <TouchableOpacity key={obj.id} activeOpacity={0.5}
            onPress={() => NavigationService.navigate('Details', {itemId: obj.id, title: obj.title.romaji, type: 'ANIME'})}>
            <View style={styles.box} >
      ...
      let trendingmanga = this.state.trendingmangadata.data.Page.media.map(obj => {
        return (
          <TouchableOpacity key={obj.id} activeOpacity={0.5}
            onPress={() => NavigationService.navigate('Details', {itemId: obj.id, title: obj.title.romaji, type: 'MANGA'})}>
            <View style={styles.box} >
      ...
      return(
        <ScrollView style={{flex: 1}} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal={true} style={styles.horizontalScroll} nestedScrollEnabled indicatorStyle='white'>
            {topanime}
          </ScrollView>
